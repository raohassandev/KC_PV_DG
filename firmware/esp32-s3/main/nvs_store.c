...
esp_err_t pvdg_nvs_load_wifi(pvdg_wifi_creds_t *out) {
  ...
  if (err == ESP_OK) err = nvs_get_str(h, "wifi_pass", out->password, &pass_len);
  nvs_close(h);
  return err;
}
...
esp_err_t pvdg_nvs_load_site_json(char **out_json) {
  ...
  *out_json = NULL;
  ...
  if (err != ESP_OK || len == 0) { nvs_close(h); return err; }
...
esp_err_t pvdg_nvs_save_site_json(const char *json) {
  ...
  if (err == ESP_OK) err = nvs_commit(h);
  nvs_close(h);
  if (err != ESP_OK) ESP_LOGW(TAG, "save site_json failed: %s", esp_err_to_name(err));
  return err;
}

esp_err_t pvdg_nvs_load_cfg_version(uint32_t *out_ver) {
  if (!out_ver) return ESP_ERR_INVALID_ARG;
  *out_ver = 0;
  nvs_handle_t h;
  esp_err_t err = nvs_open(NS, NVS_READONLY, &h);
  if (err != ESP_OK) return err;
  err = nvs_get_u32(h, "cfg_ver", out_ver);
  nvs_close(h);
  if (err == ESP_ERR_NVS_NOT_FOUND) { *out_ver = 0; return ESP_OK; }
  return err;
}
...
esp_err_t pvdg_nvs_bump_cfg_version(void) {
  ...
  if (err != ESP_OK) return err;
  uint32_t ver = 0;
  nvs_get_u32(h, "cfg_ver", &ver);
  ver++;
  err = nvs_set_u32(h, "cfg_ver", ver);
  if (err == ESP_OK) err = nvs_commit(h);
  nvs_close(h);
  return err;
}
